...
- 未来 24 小时预测概率从 `80%` 或以下上升到严格大于 `80%`（较高提醒）；
- 在没有明确公开预告时，未来 48 小时预测概率从 `70%` 或以下上升到严格大于 `70%`（提前关注）；
- 第三方接口将一条新的公开 X 消息标为明确的重置预告时，立即提醒（不受 24 小时概率阈值限制）；
- 第三方接口的 `last_reset_at` 出现更晚的时间；
- 接口连续 3 次读取失败，以及之后恢复；
- 手动触发的企业微信测试消息。

同一轮只推送优先级最高的一条：明确公开预告优先于 24 小时预测，24 小时预测优先于 48 小时预测，避免刷屏。
同一条公开预告或同一条重置记录也只推送一次。公开预告消息会包含原帖摘要和链接；它是
...
7. 再选择 `check-now` 运行一次，建立初始状态。首次只记录已有的重置时间，不会把旧记录当成新重置推送；如果当时有新的明确公开预告，或预测已越过 24/48 小时阈值，会发一次对应提醒。
...
```yaml
ALERT_THRESHOLD: "80"
ALERT_THRESHOLD_48H: "70"
```

`ALERT_THRESHOLD` 控制 24 小时提醒；`ALERT_THRESHOLD_48H` 控制 48 小时提前关注。两者都采用“严格大于”规则，例如设为 `70` 表示从 70% 或以下上穿到大于 70% 时提醒。
